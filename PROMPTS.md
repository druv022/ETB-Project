...
- **2026-04-02**: `Minimal Docker smoke test (MVP) — Implement the plan as specified` (Context: add `docker-smoke` and `docker-build-windows` jobs to CI, `.gitattributes` for shell scripts/Dockerfile, document Docker CI and local parity in `docs/DEVELOPMENT.md`, README Contributing pointer, `PROMPTS.md` entry.)
