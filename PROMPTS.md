...
- **2026-04-15**: `RAGAS evaluation (decoupled) — implement plan with synthetic PDF testset generation, HTTP collection via orchestrator /v1/chat, RAGAS scoring, per-run HTML + persistent dashboard with filters, AI Notes vs previous run, and optional git commit messages tracking deltas. Use conda etb env.` (Context: add `src/etb_project/evaluation/` package, `data/evals/` templates/history/dashboard, unit tests, and README updates.)

- **2026-04-16**: `Create a complete analysis of the gap between current implementation in the project and how latest compatible version of RAGAS should be implemented within the project. And then create a plan to implement without any gaps or errors.` (Context: compare `etb_project.evaluation` to RAGAS 0.4.x APIs; document metric initialization, `evaluate` vs `experiment`, testset generation, and a migration plan.)

- **2026-04-16**: `RAGAS 0.4.3 integration plan — Implement the plan as specified` (Context: `eval_models.py` with `llm_factory`/`embedding_factory`; `metrics_runner` with collection metric instances, `evaluate(..., llm, embeddings)`, aggregate from `scores`; tests; pin `ragas~=0.4.3`, add `pandas` to `[eval]`; README env vars for metric scoring.)
