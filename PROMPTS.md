...
- **2026-04-08**: `Plan: Orchestrator transaction path (8-point design, hybrid V1) — Implement the plan as specified` (Context: LangGraph `data_router`, `transaction_gate`, `fetch_transactions`; `transaction_parse.py` + `TransactionFetchParams`; `RAGState` / `ChatResponse` optional `clarify_gate` + `sql_meta`; merged-answer prompts; orchestrator `enable_data_router=True`; `ETB_SQL_PROMPT_MAX_ROWS`; tests; README + ORCHESTRATOR_API chat docs; add `langgraph` to `pyproject.toml` dependencies.)
