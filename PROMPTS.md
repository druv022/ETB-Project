...
- **2026-03-31**: `use conda etb env` (Context: run pytest with `conda run -n etb`; fix `test_agent_merge_documents.py` to import `merge_documents` from `etb_project.rag_document_merge` after merge helpers moved out of `agent_graph`.)

- **2026-03-31**: `Yes, proceed an dcomplete all ToDo` (Context: finish plan **tests-docs** — add `tests/test_grounded_subagent.py` (templates, session merge, writer graph submit + retrieve budget), `test_agent_subagent_finalize` in `test_agent_orchestrator.py`; document `ETB_GROUNDED_FINALIZE_MODE` / `ETB_WRITER_*` in `docs/ORCHESTRATOR_API.md`, `README.md`, `docs/ARCHITECTURE.md`; mark all plan YAML todos completed.)
